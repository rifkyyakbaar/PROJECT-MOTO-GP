  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white pt-24 pb-12 px-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-5xl font-black italic mb-2 text-blue-500 uppercase tracking-tighter">WSBK <span className="text-white">Championship</span></h1>
        <p className="text-gray-400 mb-12 text-lg">Production bike racing at its absolute best.</p>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {events.map((event) => (
              <Link href={`/event/${event.id}`} key={event.id} className="block group">
                <div className="bg-[#111] border border-gray-800 rounded-2xl flex flex-col md:flex-row overflow-hidden hover:border-blue-500 transition-all shadow-lg transform hover:-translate-y-1">
                  ...
                  <div className="bg-[#0a0a0a] md:w-32 flex flex-col items-center justify-center p-4 border-b md:border-b-0 md:border-r border-gray-800 shrink-0">
                    <span className="text-blue-500 font-black tracking-widest">{getMonth(event.date)}</span>
                  ...
                  <div className="md:w-64 p-6 flex flex-col justify-center items-end border-t md:border-t-0 md:border-l border-gray-800 bg-[#0a0a0a] z-10 shrink-0 group-hover:bg-[#111] transition-colors">
                    ...
                    <button className="w-full bg-blue-600 hover:bg-white hover:text-black text-white font-black py-3 px-4 rounded-lg italic uppercase transition-all">
                      Get Tickets Now
                    </button>
                  </div>
                </div>
          ...
          <p className="text-gray-500 italic">Belum ada jadwal WSBK yang tersedia.</p>
        )}
      </div>
    </div>
  );
