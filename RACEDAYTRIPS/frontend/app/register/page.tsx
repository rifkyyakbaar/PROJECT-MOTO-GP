  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("❌ Pendaftaran gagal. Username atau Email mungkin sudah terdaftar.");
      }
    } catch (error) {
      alert("Terjadi kesalahan sistem. Pastikan Backend Golang menyala.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-md w-full bg-[#111] border border-gray-800 rounded-3xl p-8 relative z-10 shadow-2xl">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-black italic tracking-tighter text-white mb-2">
            JOIN <span className="text-red-600">THE CLUB</span>
          </h1>
          <p className="text-gray-400 font-medium text-sm">Buat akun untuk mulai memesan tiket.</p>
        </div>

        <form onSubmit={handleRegister} className="space-y-5">
          <div>
            <label className="block text-gray-400 text-sm font-bold mb-2 uppercase tracking-wider">Username / Nama</label>
            <input 
        ...
        <p className="text-center text-gray-500 text-sm mt-8 font-medium">
          Sudah punya akun? <Link href="/login" className="text-red-500 hover:text-white transition-colors font-bold">Sign In di sini</Link>
        </p>
      </div>
    </div>
  );
