...
export default function MotoGPPage() {
  ...
  useEffect(() => {
    ...
    .then(([eventsData, packagesData]) => {
      ...
      const eventsWithStock = motogpOnly.map((ev: any) => {
  ...
  // ✅ Kamus Bendera Pintar (Disamakan dengan Halaman Utama)
  ...
  return (
    <div className="min-h-screen text-white pt-24 pb-12 px-6 relative">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <img src="/images/motogp.jpg" alt="MotoGP Background" className="w-full h-full object-cover opacity-40 blur-[2px]" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#050505]/70 via-[#050505]/90 to-[#050505]"></div>
      </div>

      <div className="max-w-5xl mx-auto relative z-10">
        <h1 className="text-5xl font-black italic mb-2 text-red-600 uppercase tracking-tighter drop-shadow-lg">MotoGP <span className="text-white">Schedule</span></h1>
        <p className="text-gray-300 mb-12 text-lg drop-shadow-md">Book official tickets for the world’s most prestigious motorcycle race before they sell out!</p>
        ...
        {loading ? (
              ...
              const cardContent = (
                  <div className={`relative rounded-2xl flex flex-col md:flex-row overflow-hidden border ${isActive ? 'border-gray-800/60 hover:border-red-600 transition-all shadow-2xl transform hover:-translate-y-1 bg-[#111]' : 'border-gray-800 bg-[#0a0a0a] opacity-50 grayscale'}`}>
                    ...
                    <div className="relative z-10 flex-1 p-6 flex flex-col justify-center">
                      <p className={`text-xs font-bold mb-2 uppercase flex items-center ${!isActive ? 'text-gray-500' : event.stock > 0 ? 'text-green-500' : 'text-red-500'}`}>
                        {isActive && <span className="w-2 h-2 rounded-full mr-2 bg-current animate-pulse"></span>}
                        {!isActive ? (isPassed ? 'EVENT FINISHED' : 'UNAVAILABLE') : event.stock > 0 ? 'SECURE YOUR TICKETS NOW!' : 'SOLD OUT'}
                      </p>
                    ...
                    {/* KANAN (Bendera Transparan & Tombol) */}
          ...
          <div className="text-center py-20 bg-[#111] rounded-3xl border border-gray-800">
            ...
            <p className="text-gray-400 italic">No MotoGP schedule available yet.</p>
          </div>
        )}
      </div>
    </div>
