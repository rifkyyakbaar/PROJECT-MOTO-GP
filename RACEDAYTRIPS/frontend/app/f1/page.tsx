...
export default function F1Page() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/events")
      .then((res) => res.json())
      .then((data) => {
        // 🛠️ PERBAIKAN DI SINI: Mencari "F1" (dari Admin) atau "Formula 1" (dari database lama)
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white pt-24 pb-12 px-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-5xl font-black italic mb-2 text-red-600 uppercase tracking-tighter">Formula 1 <span className="text-white">Schedule</span></h1>
        <p className="text-gray-400 mb-12 text-lg">Saksikan puncak balap jet darat di sirkuit ikonik dunia.</p>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {events.map((event) => (
              <Link href={`/event/${event.id}`} key={event.id} className="block group">
                <div className="bg-[#111] border border-gray-800 rounded-2xl flex overflow-hidden hover:border-red-600 transition-all shadow-lg transform hover:-translate-y-1">
                  
                  {/* BAGIAN KIRI: TANGGAL */}
                  <div className="bg-[#0a0a0a] w-32 flex flex-col items-center justify-center p-4 border-r border-gray-800 shrink-0">
                  ...
                  {/* BAGIAN TENGAH: INFO & GAMBAR BACKGROUND */}
                  ...
                  <div className="w-64 p-6 flex flex-col justify-center items-end border-l border-gray-800 bg-[#0a0a0a] z-10 shrink-0 group-hover:bg-[#111] transition-colors">
          ...
          <p className="text-gray-500 italic">Belum ada jadwal F1 yang tersedia.</p>
        )}
      </div>
    </div>
  );
